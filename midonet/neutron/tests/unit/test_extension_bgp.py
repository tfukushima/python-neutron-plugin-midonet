...
class BgpExtensionTestCase(test_api_v2_extension.ExtensionTestCase):
    ...
    def test_bgp_show(self):
        ...
        res = self.api.get(_get_path('bgps', id=str(bgp_id), fmt=self.fmt))
        self.assertEqual(exc.HTTPOk.code, res.status_int)
    ...
    def test_ad_route_list(self):
        ...
        instance = self.plugin.return_value
        instance.get_bgp_ad_routes.return_value = return_value

        res = self.api.get(_get_path('ad_routes', id=str(bgp_id),
                                     fmt=self.fmt))
        self.assertEqual(exc.HTTPOk.code, res.status_int)

        instance.get_bgp_ad_routes.assert_called_once_with(
            mock.ANY, bgp_id=str(bgp_id), fields=mock.ANY, filters=mock.ANY)
    ...
    def test_ad_route_show(self):
        ...
        res = self.api.get(_get_path('ad_routes', id=str(ad_route_id),
                                     fmt=self.fmt))
        self.assertEqual(exc.HTTPOk.code, res.status_int)
    ...
    def test_ad_route_create(self):
        ...
        res = self.api.post(_get_path('ad_routes', fmt=self.fmt),
                            ...
                            content_type='application/%s' % self.fmt)
        self.assertEqual(exc.HTTPCreated.code, res.status_int)
        instance.create_bgp_ad_route.assert_called_once_with(
            mock.ANY, bgp_id=str(bgp_id), ad_route=data)
    ...
    def test_ad_route_delete(self):
        ...
        res = self.api.delete(_get_path('ad_routes', id=str(ad_route_id)))
